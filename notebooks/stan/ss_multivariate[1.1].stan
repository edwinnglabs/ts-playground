// This is just a snapshot after introducing global trend in a way without kalman filter
data {
  // number of steps
...
transformed parameters{
  ...
  // vector[NUM_OF_STEPS+1] glb_a ;
  // vector[NUM_OF_SERIES] glb_v[NUM_OF_STEPS];
  ...
  for (t in 1:NUM_OF_STEPS) {
    ...
    // glb_v[t] = y_tran[:, t] - a[t] - glb_a[t];
    // print("glb_t: ", t, " glb_vt: ", glb_v[t]);
    // glb_F[t] = rep_vector(glb_P[t], NUM_OF_SERIES) + obs_sigma_sq;
    // glb_a[t+1] = glb_a[t] + glb_P[t] * sum(glb_v[t] ./ glb_F[t]);  
